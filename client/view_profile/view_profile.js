...
Template.view_profile_content.onRendered(function(){

  var url = window.location.href;
  var new_url = url.split("/");
  url = new_url[new_url.length - 1];
  var view_user_profile_id = Base64.decode(url);
...
Template.view_profile_content.helpers({
  ...
  show_logged_in(){
   ...
   all_friends_array.push(user_id);
         ...
         var view_user_profile = Session.get("view_user_profile_id");

            var result = feed.find({post_by: view_user_profile,post_status: 1},{sort: {created_at: -1},limit: check_limit}).fetch();
