...
Template.edit_terms_details.onDestroyed(function () {
  book_listing.stop();
});

Template.edit_terms_details.onCreated(function eventlistOnCreated(){

});

Template.edit_terms_details.onRendered(function () {
  ...
  book_listing = Meteor.subscribe("show_terms_and_condition");
  setTimeout(function () {
    ...
    // $('#loading_div').addClass('loader_visiblity_block');
      ...
      Meteor.call('display_terms_and_condition',function(error,result){
      if(error){
          console.log("Error");
      }else{
      ...
      var terms_and_condition_text = result[0].terms_and_condition_text;
      $('#terms_and_condition').trumbowyg('html',terms_and_condition_text);
 ...
 Template.edit_terms_details.helpers({
...
Template.edit_terms_details.events({
 ...
 'click #submit_terms_and_condition': function(){
    var terms_and_condition = $("#terms_and_condition").val();

    if(terms_and_condition == null || terms_and_condition == '')
              {
                $("#terms_and_condition").addClass('emptyfield2').focus();
                return false;
              }
    else
              {
...
// swal(' terms_and_condition '+terms_and_condition);
    ...
    Meteor.call('update_terms_and_condition',terms_and_condition,function(error,result){
