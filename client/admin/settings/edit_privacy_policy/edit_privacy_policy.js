...
Template.edit_privacy_policy_details.onDestroyed(function () {
  book_listing.stop();
});

Template.edit_privacy_policy_details.onCreated(function eventlistOnCreated(){

});

Template.edit_privacy_policy_details.onRendered(function () {
  ...
  book_listing = Meteor.subscribe("show_privacy_policy");
  setTimeout(function () {
    ...
    // $('#loading_div').addClass('loader_visiblity_block');
      ...
      Meteor.call('display_privacy_policy',function(error,result){
      if(error){
          console.log("Error");
      }else{
      ...
      var privacy_policy_text = result[0].privacy_policy_text;
      $('#privacy_policy').trumbowyg('html',privacy_policy_text);
 ...
 Template.edit_privacy_policy_details.helpers({
...
Template.edit_privacy_policy_details.events({
 ...
 'click #submit_privacy_policy': function(){
    var privacy_policy = $("#privacy_policy").val();

    if(privacy_policy == null || privacy_policy == '')
              {
                $("#privacy_policy").addClass('emptyfield2').focus();
                return false;
              }
    else
              {
                $("#privacy_policy").removeClass('emptyfield2');
              }
// alert(' privacy_policy '+privacy_policy);
    $('#loader_gif').removeClass('div_hide_class');
    $('#save_text').addClass('div_hide_class');

    Meteor.call('update_privacy_policy',privacy_policy,function(error,result){
