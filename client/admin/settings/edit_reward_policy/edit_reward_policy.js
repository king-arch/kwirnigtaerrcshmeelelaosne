...
Template.edit_reward_policy_details.onDestroyed(function () {
  book_listing.stop();
});

Template.edit_reward_policy_details.onCreated(function eventlistOnCreated(){

});

Template.edit_reward_policy_details.onRendered(function () {
  ...
  book_listing = Meteor.subscribe("show_reward_policy");
  setTimeout(function () {
    ...
    // $('#loading_div').addClass('loader_visiblity_block');
      ...
      Meteor.call('display_reward_policy',function(error,result){
      if(error){
          console.log("Error");
      }else{
      ...
      var reward_policy_text = result[0].reward_policy_text;
      $('#reward_policy').trumbowyg('html',reward_policy_text);
 ...
 Template.edit_reward_policy_details.helpers({
...
Template.edit_reward_policy_details.events({
 ...
 'click #submit_reward_policy': function(){
    var reward_policy = $("#reward_policy").val();

    if(reward_policy == null || reward_policy == '')
              {
                $("#reward_policy").addClass('emptyfield2').focus();
                return false;
              }
    else
              {
                $("#reward_policy").removeClass('emptyfield2');
              }
// alert(' reward_policy '+reward_policy);
    $('#loader_gif').removeClass('div_hide_class');
    $('#save_text').addClass('div_hide_class');

    Meteor.call('update_reward_policy',reward_policy,function(error,result){
