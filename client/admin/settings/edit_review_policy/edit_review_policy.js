...
Template.edit_review_policy_details.onDestroyed(function () {
  book_listing.stop();
});

Template.edit_review_policy_details.onCreated(function eventlistOnCreated(){

});

Template.edit_review_policy_details.onRendered(function () {
  ...
  book_listing = Meteor.subscribe("show_review_policy");
  setTimeout(function () {
    ...
    // $('#loading_div').addClass('loader_visiblity_block');
      ...
      Meteor.call('display_review_policy',function(error,result){
      if(error){
          console.log("Error");
      }else{
      ...
      var review_policy_text = result[0].review_policy_text;
      $('#review_policy').trumbowyg('html',review_policy_text);
 ...
 Template.edit_review_policy_details.helpers({
...
Template.edit_review_policy_details.events({
 ...
 'click #submit_review_policy': function(){
    var review_policy = $("#review_policy").val();

    if(review_policy == null || review_policy == '')
              {
                $("#review_policy").addClass('emptyfield2').focus();
                return false;
              }
    else
              {
                $("#review_policy").removeClass('emptyfield2');
              }
// alert(' review_policy '+review_policy);
    $('#loader_gif').removeClass('div_hide_class');
    $('#save_text').addClass('div_hide_class');

    Meteor.call('update_review_policy',review_policy,function(error,result){
