...
Template.campaign_completion_report_details.onDestroyed(function () {
  book_listing.stop();
  user_details_listing.stop();
  request_listing.stop();
});

Template.campaign_completion_report_details.onCreated(function eventlistOnCreated(){

});

Template.campaign_completion_report_details.onRendered(function () {
 ...
 Template.campaign_completion_report_details.helpers({
    ...
    show_reviewers_listing(){
      ...
      var result = review_details.find({content_type: "submit_review",parent_id: campaign_id, approval_status: 1 }).fetch();
      return result;
    ...
    show_book_detail_for_header(){
      var result = campaign_details.find({campaign_id: Session.get("get_campaign_id"), entry_type: {$ne: "campaign_completion_report"} }).fetch();
      return result;
    },
...
Template.campaign_completion_report_details.events({
